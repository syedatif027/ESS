...
struct ess_pollen_concentration {
	uint8_t pollen_concentration_data[3];
	wchar_t *user_desc;
	uint16_t indication;
...
static void es_elevation_init(void)
{
	...
	elevation_db->elevation_data[0] = rand();
	elevation_db->elevation_data[1] = rand();
	elevation_db->elevation_data[2] = rand();
...
static void es_pollen_concentration_init(void)
{
	...
	pollen_concentration_db->pollen_concentration_data[0] = rand();
	pollen_concentration_db->pollen_concentration_data[1] = rand();
	pollen_concentration_db->pollen_concentration_data[2] = rand();
...
static void es_magnetic_flux_3D_init(void)
{
	...
	memcpy(magnetic_flux_3D_db->user_desc, "Magnetic Flux 2D", 16);
...
static bool apparent_speed_value_calculation(void *user_data)
{
	...
	switch(apparent_speed_db->apparent_speed_tr->condition){
		...
		case 0x08 : if( pdu != apparent_speed_db->apparent_speed_data)
				notify=false;
			    break;

		default   : if( pdu == apparent_speed_db->apparent_speed_data)
				notify=false;
...
static bool temp_value_calculation(void *user_data)
{
	...
	switch(temp_db->temp_tr->condition){
		...
		case 0x08 : if( pdu != temp_db->temp_data)
				notify=false;
			    break;

		default   : if( pdu == temp_db->temp_data)
				notify=false;
...
static bool apparent_direction_value_calculation(void *user_data)
{
	...
	switch(apparent_direction_db->apparent_direction_tr->condition){
		...
		case 0x08 : if( pdu != apparent_direction_db->apparent_direction_data)
				notify=false;
			    break;

		default   : if( pdu == apparent_direction_db->apparent_direction_data)
				notify=false;
		      ...
		      uint8_t opcode, struct bt_att *att, void *user_data)
{
	...
	value[0]=dew_point_db->lower_dew_point;
	value[1]=dew_point_db->upper_dew_point;
...
static bool dew_point_value_calculation(void *user_data)
{
	...
	switch(dew_point_db->dew_point_tr->condition){
		...
		case 0x08 : if( pdu != dew_point_db->dew_point_data)
				notify=false;
			    break;

		default   : if( pdu == dew_point_db->dew_point_data)
				notify=false;
		      ...
		      uint8_t opcode, struct bt_att *att, void *user_data)
{
	...
	value[0]=elevation_db->elevation_data[0];
	value[1]=elevation_db->elevation_data[1];
	value[2]=elevation_db->elevation_data[2];
...
static bool elevation_value_calculation(void *user_data)
{
	...
	switch(elevation_db->elevation_tr->condition){
		...
		case 0x08 : if( pdu != elevation_db->elevation_data)
				notify=false;
			    break;

		default   : if( pdu == elevation_db->elevation_data)
				notify=false;
				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		elevation_db->tr_value=value[1] | value[2]<<8 ;
...
static bool gust_factor_value_calculation(void *user_data)
{
	...
	switch(gust_factor_db->gust_factor_tr->condition){
		...
		case 0x08 : if( pdu != gust_factor_db->gust_factor_data)
				notify=false;
			    break;

		default   : if( pdu == gust_factor_db->gust_factor_data)
				notify=false;
...
static bool heat_index_value_calculation(void *user_data)
{
	...
	switch(heat_index_db->heat_index_tr->condition){
		...
		case 0x08 : if( pdu != heat_index_db->heat_index_data)
				notify=false;
			    break;

		default   : if( pdu == heat_index_db->heat_index_data)
				notify=false;
...
static bool humidity_value_calculation(void *user_data)
{
	...
	switch(humidity_db->humidity_tr->condition){
		...
		case 0x08 : if( pdu != humidity_db->humidity_data)
				notify=false;
			    break;

		default   : if( pdu == humidity_db->humidity_data)
				notify=false;
...
static bool irradiance_value_calculation(void *user_data)
{
	...
	switch(irradiance_db->irradiance_tr->condition){
		...
		case 0x08 : if( pdu != irradiance_db->irradiance_data)
				notify=false;
			    break;

		default   : if( pdu == irradiance_db->irradiance_data)
				notify=false;
		      ...
		      uint8_t opcode, struct bt_att *att, void *user_data)
{
	...
	value[0]=pollen_concentration_db->pollen_concentration_data[0];
	value[1]=pollen_concentration_db->pollen_concentration_data[1];
	value[2]=pollen_concentration_db->pollen_concentration_data[2];
...
static bool pollen_concentration_value_calculation(void *user_data)
{
	...
	switch(pollen_concentration_db->pollen_concentration_tr->condition){
		...
		case 0x08 : if( pdu != pollen_concentration_db->pollen_concentration_data)
				notify=false;
			    break;

		default   : if( pdu == pollen_concentration_db->pollen_concentration_data)
				notify=false;
				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		pollen_concentration_db->tr_value=value[1] | value[2]<<8 ;
...
static bool rainfall_value_calculation(void *user_data)
{
	...
	switch(rainfall_db->rainfall_tr->condition){
		...
		case 0x08 : if( pdu != rainfall_db->rainfall_data)
				notify=false;
			    break;

		default   : if( pdu == rainfall_db->rainfall_data)
				notify=false;
...
static bool pressure_value_calculation(void *user_data)
{
	...
	switch(pressure_db->pressure_tr->condition){
		...
		case 0x08 : if( pdu != pressure_db->pressure_data)
				notify=false;
			    break;

		default   : if( pdu == pressure_db->pressure_data)
				notify=false;
				...
				void *user_data)
{
	...
	if (len<1 && len>5) {
		...
		pressure_db->tr_value=value[1] | value[2]<<8 | value[2]<<16 | value[2]<<24 ;
...
static bool true_direction_value_calculation(void *user_data)
{
	...
	switch(true_direction_db->true_direction_tr->condition){
		...
		case 0x08 : if( pdu != true_direction_db->true_direction_data)
				notify=false;
			    break;

		default   : if( pdu == true_direction_db->true_direction_data)
				notify=false;
...
static bool true_speed_value_calculation(void *user_data)
{
	...
	switch(true_speed_db->true_speed_tr->condition){
		...
		case 0x08 : if( pdu != true_speed_db->true_speed_data)
				notify=false;
			    break;

		default   : if( pdu == true_speed_db->true_speed_data)
				notify=false;
...
static bool uv_index_value_calculation(void *user_data)
{
	...
	switch(uv_index_db->uv_index_tr->condition){
		...
		case 0x08 : if( pdu != uv_index_db->uv_index_data)
				notify=false;
			    break;

		default   : if( pdu == uv_index_db->uv_index_data)
				notify=false;
...
static bool wind_chill_value_calculation(void *user_data)
{
	...
	switch(wind_chill_db->wind_chill_tr->condition){
		...
		case 0x08 : if( pdu != wind_chill_db->wind_chill_data)
				notify=false;
			    break;

		default   : if( pdu == wind_chill_db->wind_chill_data)
				notify=false;
...
static bool magnetic_declination_value_calculation(void *user_data)
{
	...
	switch(magnetic_declination_db->magnetic_declination_tr->condition){
		...
		case 0x08 : if( pdu != magnetic_declination_db->magnetic_declination_data)
				notify=false;
			    break;

		default   : if( pdu == magnetic_declination_db->magnetic_declination_data)
				notify=false;
...
static bool magnetic_flux_2D_value_calculation(void *user_data)
{
	...
	switch(magnetic_flux_2D_db->magnetic_flux_2D_tr->condition){
		...
		case 0x03 : if( pdu == magnetic_flux_2D_db->magnetic_flux_2D_xdata)
				notify=false;
			    else
			    	magnetic_flux_2D_db->magnetic_flux_2D_xdata= pdu;
			    break;

		case 0x04 : if( pdu >= magnetic_flux_2D_db->tr_value)
				notify=false;
			    break;

		case 0x05 : if( pdu > magnetic_flux_2D_db->tr_value)
				notify=false;
			    break;

		case 0x06 : if( pdu <= magnetic_flux_2D_db->tr_value)
				notify=false;
			    break;

		case 0x07 : if( pdu < magnetic_flux_2D_db->tr_value)
				notify=false;
			    break;

		case 0x08 : if( pdu != magnetic_flux_2D_db->magnetic_flux_2D_xdata)
				notify=false;
			    break;

		default   : if( pdu == magnetic_flux_2D_db->magnetic_flux_2D_xdata)
				notify=false;
			       ...
			       void *user_data)
{
	...
	if(val[0] == 0x00){
		...
		error=0;
		val[1]=(uint8_t)magnetic_flux_2D_db->tr_value;
		val[2]=(uint8_t)(magnetic_flux_2D_db->tr_value>>8);
		len=3;
	}
				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		magnetic_flux_2D_db->tr_value=value[1] | value[2]<<8 ;
...
static bool magnetic_flux_3D_value_calculation(void *user_data)
{
	...
	switch(magnetic_flux_3D_db->magnetic_flux_3D_tr->condition){
		...
		case 0x03 : if( pdu == magnetic_flux_3D_db->magnetic_flux_3D_xdata)
				notify=false;
			    else
			    	magnetic_flux_3D_db->magnetic_flux_3D_xdata= pdu;
			    break;

		case 0x04 : if( pdu >= magnetic_flux_3D_db->tr_value)
				notify=false;
			    break;

		case 0x05 : if( pdu > magnetic_flux_3D_db->tr_value)
				notify=false;
			    break;

		case 0x06 : if( pdu <= magnetic_flux_3D_db->tr_value)
				notify=false;
			    break;

		case 0x07 : if( pdu < magnetic_flux_3D_db->tr_value)
				notify=false;
			    break;

		case 0x08 : if( pdu != magnetic_flux_3D_db->magnetic_flux_3D_xdata)
				notify=false;
			    break;

		default   : if( pdu == magnetic_flux_3D_db->magnetic_flux_3D_xdata)
				notify=false;
			       ...
			       void *user_data)
{
	...
	if(val[0] == 0x00){
		...
		error=0;
		val[1]=(uint8_t)magnetic_flux_3D_db->tr_value;
		val[2]=(uint8_t)(magnetic_flux_3D_db->tr_value>>8);
		len=3;
	}
				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		magnetic_flux_3D_db->tr_value=value[1] | value[2]<<8 ;
...
{
	struct gatt_db_attribute *service, *temp,*apparent_direction,*apparent_speed,*dew_point,*gust_factor,*heat_index,*humidity,*irradiance,*rainfall,*pressure,*true_direction,*true_speed,*uv_index,*wind_chill,*magnetic_declination,*magnetic_flux_2D,*magnetic_flux_3D;
	bt_uuid_t uuid;
