				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		error=0;
		elevation_db->elevation_tr->trigger_inactive=false;		 /*if trigger is value based */
		elevation_db->tr_value=value[1] | value[2]<<8 | value[3]<<16;
				...
				void *user_data)
{
	...
	if (len<1 && len>4) {
		...
		error=0;
		pollen_concentration_db->pollen_concentration_tr->trigger_inactive=false;		/* if trigger is value based */
		pollen_concentration_db->tr_value=value[1] | value[2]<<8 | value[3]<<16 ;
	...
	*/
	...
	switch(magnetic_flux_2D_db->magnetic_flux_2D_tr->condition){
		...
		case 0x03 : if( pdu[0] == magnetic_flux_2D_db->magnetic_flux_2D_xdata && pdu[1]== magnetic_flux_2D_db->magnetic_flux_2D_ydata)
				notify=false;
			    else if(pdu[0]==magnetic_flux_2D_db->magnetic_flux_2D_xdata && pdu[1]!=magnetic_flux_2D_db->magnetic_flux_2D_ydata)
			    	magnetic_flux_2D_db->magnetic_flux_2D_ydata= pdu[1];
			    else if(pdu[0]!=magnetic_flux_2D_db->magnetic_flux_2D_xdata && pdu[1]==magnetic_flux_2D_db->magnetic_flux_2D_ydata)
			    	magnetic_flux_2D_db->magnetic_flux_2D_xdata= pdu[0];
			    break;

		case 0x04 : if( pdu[0]>=magnetic_flux_2D_db->tr_valuex && pdu[1]>=magnetic_flux_2D_db->tr_valuey)
				notify=false;
			    else if(pdu[0]>=magnetic_flux_2D_db->tr_valuex && pdu[1]<magnetic_flux_2D_db->tr_valuey)
			    	pdu[0]=magnetic_flux_2D_db->magnetic_flux_2D_xdata;
			    else if(pdu[0]<magnetic_flux_2D_db->tr_valuex && pdu[1]>=magnetic_flux_2D_db->tr_valuey)
			    	pdu[1]=magnetic_flux_2D_db->magnetic_flux_2D_ydata;
			    break;
	...
	*/
	...
	switch(magnetic_flux_3D_db->magnetic_flux_3D_tr->condition){
		...
		case 0x04 : if( pdu[0] >= magnetic_flux_3D_db->tr_valuex && pdu[1] >= magnetic_flux_3D_db->tr_valuey && pdu[2] >= magnetic_flux_3D_db->tr_valuez )
				notify=false;
			    else if(pdu[0] >= magnetic_flux_3D_db->tr_valuex && pdu[1] < magnetic_flux_3D_db->tr_valuey && pdu[2] < magnetic_flux_3D_db->tr_valuez )
				pdu[0]=magnetic_flux_3D_db->magnetic_flux_3D_xdata;
			    else if(pdu[0] < magnetic_flux_3D_db->tr_valuex && pdu[1] >= magnetic_flux_3D_db->tr_valuey && pdu[2] < magnetic_flux_3D_db->tr_valuez )
				pdu[1]=magnetic_flux_3D_db->magnetic_flux_3D_ydata;
			    else if(pdu[0] < magnetic_flux_3D_db->tr_valuex && pdu[1] < magnetic_flux_3D_db->tr_valuey && pdu[2] >= magnetic_flux_3D_db->tr_valuez )
				pdu[2]=magnetic_flux_3D_db->magnetic_flux_3D_zdata;
			    else if(pdu[0] >= magnetic_flux_3D_db->tr_valuex && pdu[1] >= magnetic_flux_3D_db->tr_valuey && pdu[2] < magnetic_flux_3D_db->tr_valuez ){
				pdu[0]=magnetic_flux_3D_db->magnetic_flux_3D_xdata;
				pdu[1]=magnetic_flux_3D_db->magnetic_flux_3D_ydata;
			    }
		...
		case 0x08 : if( pdu[0] != magnetic_flux_3D_db->tr_valuex && pdu[1] != magnetic_flux_3D_db->tr_valuey && pdu[2] != magnetic_flux_3D_db->tr_valuez)
				notify=false;
			    break;
